...
import { ArtNFT as ApiBackground } from "@/services/api";
...
const BackgroundModal: React.FC<BackgroundModalProps> = ({
  ...
  const price =
    typeof background.price === "string"
      ? background.price
      : background.price.toString();

  // Get either artist or artistAddress
  const artist = isApiBackground
    ? `${(background as ApiBackground).artistAddress.slice(0, 6)}...${(
        background as ApiBackground
      ).artistAddress.slice(-4)}`
    : (background as GridBackground).artist || "Unknown Artist";

  // Use title for ApiBackground, name for GridBackground
  const title = isApiBackground
    ? (background as ApiBackground).giftCardCategoryId
              ...
              <div className="p-8 -mt-16 relative z-20">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-gray-400 text-lg">By {artist}</p>
                  </div>
                  ...
                  <ul className="space-y-3 text-base text-gray-300">
                    <li className="flex justify-between items-center">
                      ...
                      <span className="text-indigo-400 font-medium">
                        {isApiBackground
                          ? (background as ApiBackground).giftCardCategoryId
                          : (background as GridBackground).category}
                      </span>
                    </li>
                    <li className="flex justify-between items-center">
                      <span>Price</span>
